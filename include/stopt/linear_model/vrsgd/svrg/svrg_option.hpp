...
namespace stopt
{
  ...
  public:
    ...
    inline bool set_args( int argc, char **argv )
    {
      cxxopts::Options opt("SVRG","An Option for SVRG");
      opt.add_options()
	...
	("m", "max inner epoch", cxxopts::value<int_type>() )
	...
	if( j.find("m")               != j.end() ){ m               = j["m"]; }
      ...
      if( opt.count("m")              ){ m              = opt["m"].as<int_type>(); }
